...
export class TeamValidator {
	...
	getValidationSpecies(set: PokemonSet): { outOfBattleSpecies: Species, tierSpecies: Species } {
		...
		if (ability.id === 'battlebond' && toID(species.baseSpecies) === 'greninja'  && tierSpecies.name !== "Greninja-Omega" && tierSpecies.name !== "Greninja-Delta" && this.format.mod !== 'gen9legendsou') {
	...
	validateSet(set: PokemonSet, teamHas: AnyObject): string[] | null {
		...
		if (set.name && set.name.length > 20) {
			if (set.name === set.species) {
				...
				problems.push(`Nickname "${set.name}" too long (should be 20 characters or fewer)`);
			}
		}
	...
	validateForme(set: PokemonSet) {
		...
		if (species.baseSpecies === "Greninja" && toID(set.ability) === 'battlebond'  && species.name !== "Greninja-Omega" && species.name !== "Greninja-Delta") {
	...
	checkSpecies(set: PokemonSet, species: Species, tierSpecies: Species, setHas: { [k: string]: true }) {
		...
		if (tierSpecies.baseSpecies === 'Greninja' && toID(set.ability) === 'battlebond' && tierSpecies.name !== "Greninja-Omega" && tierSpecies.name !== "Greninja-Delta") {
