...
export class Pokemon {
	...
	*/
	...
	ignoringAbility() {
		...
		if (this.hasItem('Ability Shield') || this.ability === ('neutralizinggas' as ID)|| this.ability === ('moonwake' as ID) || this.ability === ('dawnbreak' as ID)) return false;
		for (const pokemon of this.battle.getAllActive()) {
			// can't use hasAbility because it would lead to infinite recursion
			if (pokemon.ability === ('neutralizinggas' as ID) || pokemon.ability === ('moonwake' as ID) && !pokemon.volatiles['gastroacid'] &&
	...
	isGrounded(negateImmunity = false) {
		...
		if (this.abilityState.surgedelugeActive) return false;
	...
	runEffectiveness(move: ActiveMove) {
		...
		if (this.hasAbility('Icebound') && !this.battle.suppressingAbility(this)) {
			if (this.battle.field.isWeather('snowscape') || this.battle.field.isWeather('hail')) {
				if (move.category !== 'Status' && move.id !== 'struggle' && this.runImmunity(move.type)) {
					if (totalTypeMod > 0) return 0;
				}
			}
