...
export const Abilities: import("../sim/dex-abilities").AbilityDataTable = {
	...
	battlebond: {
		onSourceAfterFaint(length, target, source, effect) {
			...
			if (
				...
				this.add("-activate", source, "ability: Battle Bond");
				source.formeChange("Greninja-Omega-Ash", this.effect, true);
			}
		},
	...
	covenant: {
		...
		onResidual(pokemon) {
			if (
				...
				pokemon.getAbility().id === "covenant"
			) {
	...
	embodyaspectstormpeak: {
		onStart(pokemon) {
			if (
				pokemon.baseSpecies.name === "Ogerpon-Stormpeak-Tera" &&
	...
	merciless: {
		onModifyCritRatio(critRatio, source, target) {
			if (
				(target && ["psn", "tox"].includes(target.status)) ||
		...
		onStart(pokemon) {
			if (pokemon.hp >= pokemon.maxhp / 2) {
				...
				if (pokemon.abilityState.monotype === true) {
					...
					pokemon.storedStats["spa"] >= pokemon.storedStats["spe"]
				) {
					this.hint(
	...
	shelteredslumber: {
		...
		onStart(pokemon) {
			if (
				pokemon.baseSpecies.baseSpecies !== "Polfeegeist" ||
				pokemon.transformed
			)
				return;
			if (pokemon.status === "slp") {
	...
	stickyhold: {
		onTakeItem(item, pokemon, source) {
			...
			if (
				...
				this.activeMove.id === "seasonalblessing" ||
				this.activeMove.id === "whiskaway" //fuck this hardcode
			) {
		...
		onAfterMoveSecondarySelf(source, target, move) {
			if (
				...
				this.damage(
					...
					source,
					source,
					this.dex.abilities.get("Swordmaster")
				);
			}
		},
		onModifySpe(spe, pokemon) {
	...
	icebound: {
		...
		onDamage(damage, target, source, effect) {
			if (effect && this.field.isWeather('snow') || this.field.isWeather('hail') && effect.id === 'stealthrock' || effect.id === 'gmaxsteelsurge' || effect.id === 'spikes') {
				return false;
			}
		},
		flags: {},
