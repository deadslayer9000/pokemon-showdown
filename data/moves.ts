...
export const Moves: import('../sim/dex-moves').MoveDataTable = {
	...
	aurasphere: {
		...
		pp: 20,
		priority: 0,
		flags: { protect: 1, mirror: 1, distance: 1, metronome: 1, bullet: 1, pulse: 1, crescent: 1 },
		secondary: null,
		target: "any",
		type: "Fighting",
		contestType: "Beautiful",
	...
	darkpulse: {
		...
		pp: 15,
		priority: 0,
		flags: { protect: 1, mirror: 1, distance: 1, metronome: 1, pulse: 1, crescent: 1 },
		secondary: {
			chance: 20,
			volatileStatus: 'flinch',
		},
	...
	flash: {
		...
		pp: 20,
		priority: 0,
		flags: { protect: 1, reflectable: 1, mirror: 1, metronome: 1, crescent: 1 },
		boosts: {
			accuracy: -1,
		},
		secondary: null,
	...
	gmaxsteelsurge: {
		...
		condition: {
			...
			onSwitchIn(pokemon) {
				if (pokemon.hasItem('heavydutyboots') || pokemon.hasAbility("Breach")) return;
	...
	sandstormfury: {
		...
		onTry(source, target, move) {
			...
			this.hint("Only a Pokemon whose form is Delta-Hoopa-Unbound can use this move.");
			return null;
		},
		secondary: null,
		target: "normal",
	...
	solarbeam: {
		...
		pp: 10,
		priority: 0,
		flags: { charge: 1, protect: 1, mirror: 1, metronome: 1, nosleeptalk: 1, failinstruct: 1, crescent: 1 },
		onTryMove(attacker, defender, move) {
			if (attacker.removeVolatile(move.id)) {
				return;
			}
	...
	spikes: {
		...
		condition: {
			...
			onSwitchIn(pokemon) {
				if (!pokemon.isGrounded() || pokemon.hasItem('heavydutyboots') || pokemon.hasAbility("Breach")) return;
	...
	toxicspikes: {
		...
		condition: {
			...
			onSwitchIn(pokemon) {
				...
				} else if (pokemon.hasType('Steel') || pokemon.hasItem('heavydutyboots') || pokemon.hasAbility("Breach")) {
					// do nothing
